...
import {
  ...
  AlertCircle, ChevronRight, ArrowLeft, MessageCircle
} from "lucide-react";
...
const MOCK_ORDERS: MockOrder[] = [
  ...
  {
    id: "ORD-G7H8I9", listingTitle: "GG Marmont matelassé pequeño", brand: "Gucci",
    photo: "https://images.unsplash.com/photo-1584917865442-de89df76afd3?w=200&q=80",
    totalCLP: 661500, status: "completed", updatedAt: "10 may",
    events: [
...
const STATUS_MAP: Record<OrderStatus, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  in_transit_to_warehouse: { label: "En camino a verificación", color: "text-trust", icon: <Truck className="h-3.5 w-3.5" /> },
    ...
    <div className="app-shell pb-24">
      ...
      <div className="px-4 pt-3 space-y-3">
        {MOCK_ORDERS.map((o) => {
          ...
          return (
            <button key={o.id} onClick={() => setSelected(o.id)} className="w-full text-left rounded-2xl border border-border bg-card overflow-hidden active:scale-[0.98] transition-transform">
              <div className="flex gap-3 p-3">
                <img src={o.photo} className="h-16 w-16 rounded-xl object-cover shrink-0" alt="" />
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{o.brand}</p>
        ...
        {MOCK_ORDERS.length === 0 && (
          <div className="rounded-2xl border border-dashed border-border p-12 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Explora nuestro catálogo y encuentra algo que te guste.</p>
            <Link to="/" className="mt-4 inline-flex rounded-full bg-primary px-5 py-2.5 text-xs font-semibold text-primary-foreground">
              Explorar
            </Link>
          </div>
        )}
  ...
  return (
    <div className="app-shell pb-28">
      <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-background px-3 safe-top">
        ...
        <div>
          <h1 className="font-display text-base font-semibold">#{order.id}</h1>
      ...
      <div className="space-y-4 px-4 pt-4">
        ...
        <div className="flex gap-3 rounded-2xl border border-border p-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{order.brand}</p>
        ...
        {/* Estado */}
        <div className={`flex items-center gap-2 rounded-xl border border-border p-3.5 ${info.color}`}>
          {info.icon}
        ...
        {/* Timeline */}
        <div className="rounded-2xl border border-border p-4">
          <p className="mb-4 text-sm font-semibold">Seguimiento del pedido</p>
          {order.events.map((e, i) => (
            <div key={i} className="flex gap-3 pb-4 last:pb-0">
              <div className="flex flex-col items-center">
                <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${e.done ? "border-trust bg-trust" : "border-border bg-background"}`}>
        ...
        {/* Soporte */}
        <button className="flex w-full items-center gap-3 rounded-2xl border border-border p-4 text-left hover:bg-muted/50 transition-colors">
        ...
        {/* Disputa */}
        {(order.status === "delivered" || order.status === "completed") && !disputeOpen && (
        ...
        {disputeOpen && (
          <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-4 space-y-3">
            <p className="text-sm font-semibold text-destructive">¿Cuál es el problema?</p>
            {["No es como se describe", "Artículo sospechoso / podría ser falso", "Llegó dañado en el envío", "Otro"].map((r) => (
              <button key={r} className="w-full rounded-xl border border-destructive/20 bg-background px-4 py-3 text-left text-sm font-medium text-destructive hover:bg-destructive/5 transition-colors">
                {r}
              </button>
            ))}
            <button onClick={() => setDisputeOpen(false)} className="w-full text-center text-xs text-muted-foreground">
              Cancelar
            </button>
          </div>
        )}
