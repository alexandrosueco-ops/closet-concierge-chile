...
export function LegalLayout({ title, body }: { title: string; body: string }) {
  return (
    <div className="app-shell pb-12">
      ...
      <article className="px-5 py-6 text-sm leading-relaxed text-muted-foreground whitespace-pre-line">
        {body}
      </article>
    </div>
  );
...
export const Route = createFileRoute("/legal/terms")({
  head: () => ({ meta: [{ title: "Términos — VeriCloset" }] }),
  component: () => (
    <LegalLayout
      title="Términos y condiciones"
      body={`Bienvenido a VeriCloset. Al usar la plataforma aceptas estos términos.\n\n1. Solo se permiten artículos auténticos de marcas premium aprobadas.\n2. Toda venta pasa por verificación en bodega antes de la entrega.\n3. Los pagos quedan en garantía y se liberan tras entrega + 48h sin disputa.\n4. Falsificaciones resultan en baneo permanente.`}
    />
  ),
});
