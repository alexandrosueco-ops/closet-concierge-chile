...
export function BottomTabs() {
  ...
  return (
    // Se oculta en desktop (la sidebar lo reemplaza) y en landing page
    <nav className="bottom-tabs-mobile fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-background/95 backdrop-blur safe-bottom md:hidden">
          ...
          return (
            <Link key={to} to={to as "/"}
              ...
              <div className={cn(
                ...
                primary && "bg-primary text-primary-foreground scale-110",
                active && !primary && "bg-muted",
              )}>
                <Icon className="h-5 w-5" strokeWidth={active ? 2.4 : 2} />
              </div>
              <span className={active ? "font-black" : ""}>{label}</span>
            </Link>
          );
        })}
      </div>
